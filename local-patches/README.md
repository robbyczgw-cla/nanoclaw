...
| 23 | `src/channels/chat-sdk-bridge.ts` + `container/agent-runner/src/hooks/tool-visibility.ts` | UTF-16-Surrogate-Safety: halbierter Emoji in einer Tool-Vis-Preview vergiftete den Bridge-Accumulator und blockierte via ungeschütztem Pre-Send-Flush JEDE weitere Nachricht des Chats (`text must be encoded in UTF-8`). safeSlice + toWellFormedText-Chokepoint + try/catch um finalizeToolVis | candidate (bridge part) | 🟢 local-only |
