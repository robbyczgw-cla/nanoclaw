...
- **A half-emoji in a tool-vis preview can no longer brick a whole chat (local).** A preview truncation in `tool-visibility.ts` cut an emoji between its UTF-16 surrogate halves; the lone surrogate is invalid UTF-8, Telegram rejected every flush of that bubble (`text must be encoded in UTF-8`) — and because the bridge flushes the tool-vis bubble UNGUARDED before every real message, every subsequent reply on that chat was dropped after 3 retries until a host restart (Andy's DM was silent for ~3h on 2026-07-07; 105 such errors since 2026-06-24). Three-layer fix: surrogate-safe `safeSlice()` for all preview truncations (container), `toWellFormedText()` sanitizer at the bridge's outbound choke point + surrogate-safe hard cut in `splitForLimit` (host), and the pre-send `finalizeToolVis` is now try/caught so a broken bubble can never block the real reply again. Tracked as `local-patches/23-utf8-surrogate-safety.md`.
- **Telegram sends no longer rejected with `Can't find end of Underline entity` / `Can't find end of a URL` (local).** Fable/Sonnet-style output tripped two `@chat-adapter/telegram` converter bugs: (1) the pre-send safe-boundary trimmer counts `_`/`*`/`~`/backticks inside link URLs (where Telegram's spec leaves them unescaped) and cut the message MID-URL on odd counts; (2) nested/unbalanced `__…_` emphasis rendered as adjacent underscores, which Telegram tokenizes as an unclosed underline entity. Opus' style never hit either. Fix is input-side in our `transformOutboundText` hook (`hardenForTelegramV2`): percent-encode entity-marker chars inside URLs, rewrite balanced `__x__` to `**x**`, escape stray `__`. Plain-text fallback unchanged. Regression tests run both real failure payloads through the real converter. Both root causes noted as vercel/chat upstream candidates. Tracked as `local-patches/22-telegram-v2-entity-hardening.md`.
- **Unwrapped agent replies are now fallback-delivered instead of dropped (local).** Recurring drops where the agent's `<message to="…">` reply never reached the user turned out to be model-side, not code-side: verified against raw Claude API transcripts, in 8/9 failing turns the model never typed the `<message>` wrapper at all after long tool chains (the PATCH 11 accumulation and PATCH 12/13 tolerant parser lose nothing). Fix: the `result` event now carries the turn's final text chunk (`lastText`); when a turn ends with zero deliverable blocks, `salvageUnwrappedReply()` cleans it (tag remnants, `<internal>` stripped) and the poll-loop delivers it straight to the triggering channel. The PATCH 12 re-prompt loop now fires only for unknown-destination blocks; `<internal>`-only endings ("already sent via send_message tool") are treated as intentional no-reply turns instead of being nudged into apology noise. Takes effect on next container respawn (src is volume-mounted). Tracked as `local-patches/21-unwrapped-reply-fallback.md`.
- **Sonnet-5 no longer loops on `ToolSearch` to find `send_message` (local).** With ~15 nanoclaw MCP tools + SDK builtins, the Agent SDK defers lower-frequency tools behind on-demand `ToolSearch` — and `send_message`, needed on nearly every turn, got swept into the deferred set. Opus 4.8 navigated the search→load→call dance fine; Sonnet-5 looped on it (repeated `ToolSearch select:…send_message`, replies sometimes not delivered). Fix is selective, not a sledgehammer: `_meta: {'anthropic/alwaysLoad': true}` set directly on the 4 core reply tools (`send_message`, `send_file`, `edit_message`, `add_reaction`) in `core.ts` — the same field the SDK's own `tool({alwaysLoad})` helper sets internally. The other ~11 nanoclaw tools (scheduling, agent creation, self-mod, …) stay deferred exactly as before. Measured cost: ~361 tokens added to the eager tool block (real tokenizer count), cache-amortized. Live-verified on Sonnet-5: zero `ToolSearch` calls for replies, `schedule_task`/`cancel_task` still correctly defer. Tracked as `local-patches/20-eager-core-reply-tools.md`.
- **Agent-runner SDK + Claude Code CLI bumped to match upstream `nanocoai/nanoclaw@91ebc9d`.** `@anthropic-ai/claude-agent-sdk` `^0.3.170 -> ^0.3.197`, `@anthropic-ai/sdk` `^0.100.0 -> ^0.108.0` (`container/agent-runner/package.json`), `@anthropic-ai/claude-code` `2.1.170 -> 2.1.197` (`container/cli-tools.json`, baked into the container image). No code changes required — agent-runner typechecks clean and all 143 `bun test` cases pass against the new SDK. Base image and the 4 per-group custom images (Andy, Shopify, Gildenmeister, wowdev) were rebuilt and confirmed at `claude 2.1.197`.
- **Rich rendering for headings / `<details>` / dividers / block math / task lists (local).** Extends table routing (PATCH 16): a message carrying a construct MarkdownV2 cannot render now auto-routes to Bot API 10.1 `sendRichMessage`. Plain bold/bullets/quotes stay on MarkdownV2. Block-math-inside-`<details>` (a Telegram-Desktop crash shape) is excluded. Toggle `TELEGRAM_RICH_CONSTRUCTS=false`. Generic → upstream-PR candidate. Tracked as `local-patches/18-telegram-rich-constructs.md`.

- **Native Telegram tables (local).** GFM pipe tables sent through the agent now render as real, bordered, selectable Telegram tables via Bot API 10.1 `sendRichMessage` instead of degrading to a monospace code block under MarkdownV2. A **table-primary** message auto-routes to the rich path (`telegram-rich-message.ts`); any failure falls back transparently to MarkdownV2 — the message is never lost. Toggle `TELEGRAM_RICH_TABLES=false`. Generic (no local-feature dependency) → upstream-PR candidate. Tracked as `local-patches/16-telegram-rich-tables.md`.
- **Tool-visibility timeline collapses into a `<details>` fold on turn-end (local).** Tool calls still stream live (PATCH 09); when the answer lands, the bubble is edited once into a compact `🔧 N Tool-Calls — aufklappen` summary with the per-call timeline one tap away — instead of a wall of `🔧` lines burying the reply on mobile. Renders via Bot API 10.1 `editMessageText`+`rich_message`; any failure falls back to the expanded bubble (PATCH 09 behaviour). Toggle `TELEGRAM_TOOLVIS_COLLAPSE=false`. Rides on the local tool-vis feature → not upstream-bound. Tracked as `local-patches/17-toolvis-collapse-fold.md`.

- **Local default agent model bumped to `claude-opus-4-8` (was `claude-opus-4-7[1m]`).** `claude.ts` carries a local hardcoded fallback for groups with no `model` set (upstream has none — it passes `this.model` and lets the SDK default). The literal was stale; `claude-opus-4-8` is 1M-context by default at the same price, so no `[1m]` suffix is needed. The pre-existing 900k auto-compact window (upstream `165000`) is now documented alongside it. Only affects groups with `model = NULL`; explicitly-configured groups (e.g. Andy on `claude-opus-4-8`) are unchanged. Tracked as `local-patches/15`.
- **`/learn` now available as a runtime skill inside agent containers (local).** PR #2843 added `/learn` at `.claude/skills/learn/` (the host/dev level), which is not mounted into agent containers — only `container/skills/` is (`-> /app/skills`, read-only). Copied the skill to `container/skills/learn/` so it auto-selects for every `skills:"all"` group on next spawn (no image rebuild, no orchestrator restart). Agents (e.g. Andy) can now invoke `/learn`. Tracked as `local-patches/14-learn-container-skill.md`.
